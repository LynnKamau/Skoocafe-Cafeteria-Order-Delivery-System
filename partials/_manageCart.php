...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['checkout'])) {
        ...
        if (password_verify($password, $passRow['password'])){ 
            $sql = "INSERT INTO `orders` (`userId`, `building`, `floor`, `phoneNo`, `amount`, `paymentMode`, `orderStatus`, `orderDate`) VALUES ('$userId', '$address', '$zipcode', '$phone', '$amount', '0', '0', current_timestamp())";
            $result = mysqli_query($conn, $sql);
            ...
            if ($result){
                ...
                echo '<script>alert("Thanks for ordering with us. Your order id is ' .$orderId. '.");
                    window.location.href="http://localhost/SkoocafeProject/index.php";  
                    </script>';
                    exit();
            }
        } 
